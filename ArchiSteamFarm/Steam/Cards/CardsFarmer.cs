...
public sealed class CardsFarmer : IAsyncDisposable, IDisposable {
	...
	private async Task<bool> FarmFastMultiple(IReadOnlyCollection<Game> games) {
		...
		if (await FarmingFastResetSemaphore.WaitAsync((ASF.GlobalConfig?.FarmFastMultipleDelay ?? GlobalConfig.DefaultFarmFastMultipleDelay) * 60 * 1000).ConfigureAwait(false)) {
			success = KeepFarming;
		}
	...
	private async Task<bool> FarmFast(IReadOnlyCollection<Game> games) {
		...
		if (result) {
			...
			if (gamesToSolo.Count > 0) {
				if (await FarmingFastResetSemaphore.WaitAsync((ASF.GlobalConfig?.FarmFastDelay ?? GlobalConfig.DefaultFarmFastDelay) * 1000).ConfigureAwait(false)) {
					result = KeepFarming;
				}
